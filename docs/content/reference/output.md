...
describes what users get. Pick a format with `-o`, or let digimon choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
digimon <command> -o table     # a rounded, color-aware grid for reading
digimon <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
digimon <command> -o jsonl     # one JSON object per line, for piping
digimon <command> -o json      # a single JSON array
digimon <command> -o csv       # spreadsheet friendly
digimon <command> -o tsv       # tab-separated
digimon <command> -o url       # just the URL column
digimon <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
digimon <command> --fields id,title,url
```
...
```bash
digimon <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
digimon <command>            # a table, because this is a terminal
digimon <command> | wc -l    # JSONL, because this is a pipe
```
